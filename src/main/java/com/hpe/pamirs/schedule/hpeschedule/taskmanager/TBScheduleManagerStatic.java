 ...
 * @author gaojy
 *
 * @Create_Date : 2017年8月31日下午2:17:53
 * @Update_Date :
 */
public class TBScheduleManagerStatic extends TBScheduleManager {
  ...
  public TBScheduleManagerStatic(TBScheduleManagerFactory aFactory, String baseTaskType,
      String ownSign, IScheduleDataManager aScheduleCenter) throws Exception {
  ...
  public void initial() throws Exception {
    new Thread(this.currenScheduleServer.getTaskType() + "_" + this.currenScheduleServer.toString()
        + "-StartProcess") {
      public void run() {
        try {
          ...
          while (!isRuntimeInfoInitial) {
            ...
            try {
              ...
              isRuntimeInfoInitial =
                  scheduleCenter.isInitialRunningInfoSucuss(currenScheduleServer.getBaseTaskType(),
  ...
  /**
   * 定时向数据配置中心更新当前服务器的心跳信息 如果发现本次更新的时间如果已经超过了，服务器死亡的心跳时间，则不能向服务器更新信息 而应该当做新的服务器，进行重新注册
   */
  @Override
  public void refreshScheduleServerInfo() throws Exception {
    try {
      ...
      // 判断是否需要重新加载任务队列，避免任务处理不必要的检查和等待
      ...
      if (this.isPauseSchedule == true || this.processor != null && processor.isSleeping() == true) {
        // 如果服务已经暂停，则需要重新定时更新cur_server 和req_server
        // 如果服务没有暂停，一定不能调用
        this.getCurrentScheduleTaskItemListNow();
      }

    } catch (Exception e) {
      // 清除内存中所有已经取得的数据和任务队列，避免心跳线程失败时候导致的数据重复
      this.clearMenoInfo();
  ...
  /**
   * 版本兼容判断 在leader重新分配任务，在每个server释放原来占有的任务项时，会修改这个版本号
   * 
   * @return
   * @throws Exception
   */
  ...
  /**
   * 根据当前调度服务器的信息，重新计算分配所有的调度任务 任务的分配是需要加锁，避免数据分片错误。 为了避免数据锁带来的负面作用，通过版本号来达到锁的目的
   * 
   * 1.获取任务状态的版本号 2.获取所有服务器注册信息和任务队列信息 3.清除已经超过心跳周期的服务器注册信息 4.重新计算任务分配 5.更新任务状态的版本号【乐观锁】
   * 6.更新任务队列的分配信息
   */
  @Override
  public void assignScheduleTask() throws Exception {
    ...
    // 设置初始化成功的标准，避免在leader装换的时候，新增的线程组初始化失败
    ...
    // 返回清理taskitem的cur_server个数
    scheduleCenter.clearTaskItem(this.currenScheduleServer.getTaskType(), serverList);
    scheduleCenter.assignTaskItem(this.currenScheduleServer.getTaskType(),
        this.currenScheduleServer.getUuid(), this.taskTypeInfo.getMaxTaskItemsOfOneThreadGroup(),
        serverList);
  }

  /**
   * 重新加载当前服务器的任务队列 1.释放当前服务器持有，但是其他服务器进行申请的任务队列 2.重新获取当前服务器的处理队列
   * 
   * 为了避免此操作的过度，阻塞真正的数据处理能力。系统设置一个重新转载的频率，列如1分钟。
   * 
   * 特别注意： 此方法的调用必须是在当前所有的任务都处理完毕后才能调用，否则是否任务队列后可能数据被重复处理
   */
  public List<TaskItemDefine> getCurrentScheduleTaskItemList() {
    try {
      if (this.isNeedReloadtaskItem == true) {
        // 特别需要注意：需要判断数据队列是否已经空了，否则可能在队列切换的时候导致数据重复处理
  ...
  // 由于上面的数据执行时有使用到Synchronized 但是心跳线程并没有对应加锁
  ...
  protected synchronized List<TaskItemDefine> getCurrentScheduleTaskItemListNow() throws Exception {
    // 如果已经稳定了，理论上不需要加载去扫描所有的叶子节点
    try {
      Map<String, Stat> statMap =
          this.scheduleCenter.getCurrentServerStatList(this.currenScheduleServer.getTaskType());

      // server 下面的机器节点的运行时环境是否在刷新
    ...
    // 获取最新的版本号
    this.lastFetchVersion =
        this.scheduleCenter.getReloadTaskItemFlag(this.currenScheduleServer.getTaskType());
    log.debug("this.currenScheduleServer.getTaskType()=" + this.currenScheduleServer.getTaskType()
        + ", need reload=" + isNeedReloadtaskItem);

    try {
      // 是否被人申请的队列 即req_server是否存在数据 其他server请求
      this.scheduleCenter.releaseDealTaskItem(this.currenScheduleServer.getTaskType(),
          this.currenScheduleServer.getUuid());

      // 重新查询当前服务器能够处理的队列
      // 为了避免在休眠切换的过程中出现队列瞬间不一致，先清除内存中的队列
      this.currentTaskItemList.clear();
      // 此处加载了任务队列
      this.currentTaskItemList =
          this.scheduleCenter.reloadDealTaskItem(this.currenScheduleServer.getTaskType(),
              this.currenScheduleServer.getUuid());

      // 如果超过20个心跳周期还没有获取到调度队列，则报警
      if (this.currentTaskItemList.size() == 0
          && scheduleCenter.getSystemTime() - this.lastReloadTaskItemListTime > this.taskTypeInfo
              .getHeartBeatRate() * 20) {
      ...
      this.lastFetchVersion = -1; // 必须把版本号设置-1，避免任务加载失败
      if (e instanceof Exception) {
        throw (Exception) e;
      } else {
