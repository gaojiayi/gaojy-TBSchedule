...
public class ScheduleServer {
   ...
   * FetchDataCount=4430,FetcheDataNum=438570,DealDataSucess=
   * 438570,DealDataFail=0,DealSpendTime=651066
   */
  ...
  public static ScheduleServer createScheduleServer(IScheduleDataManager aScheduleCenter,
      String aBaseTaskType, String aOwnSign, int aThreadNum) {
    ScheduleServer result = new ScheduleServer();
    // 初始化原始任务类型
    result.baseTaskType = aBaseTaskType;
    result.ownSign = aOwnSign;
    result.taskType = ScheduleUtil
            .getTaskTypeByBaseAndOwnSign(aBaseTaskType, aOwnSign);
    ...
    result.version = 0;
    result.uuid = result.ip + "$" + (UUID.randomUUID().toString().replaceAll("_", ""));
